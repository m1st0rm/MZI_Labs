...
def bin_to_hex_string(byte_content: bytes) -> str:
...
def LPSX(smth, smth2):
    return l(L(P(S(x_change(smth, smth2)))))
...
def usage():
    print("Usage: python gost_3411.py <file> [<file> ...]")
    sys.exit()
...
def main():
    ...
    for filename in sys.argv[1:]:
        try:
            ...
            content = bin_to_hex_string(content)
            content = hex_bin(content)
            print(f"{stribog(content, 1)} {filename}")
